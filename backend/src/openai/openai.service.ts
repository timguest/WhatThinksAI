...
export class OpenaiService {
  ...
  async findInfoWithGPT({ data: dataset, body: requestBody, count: length }: any) {
    try {
      const arrayOfPromises = dataset.map(async (item) => {
        ...
        const { formatted_prompt } = instructions;
        console.log('Formatted System Prompt:', formatted_prompt);
  ...
  async findPaid(query_id: string) {
    try {
      ...
      const response = await this.findInfoWithGPT({
        data: personas,
        body: query_body?.data,
